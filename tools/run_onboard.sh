...
build_aicpu() {
  ...
  cmake -B "$BUILD_DIR" -S "${ROOT}/cmake/aicpu" \
    ...
    -DCMAKE_C_FLAGS="-DESL_PROXY_ONBOARD -DESL_LANE_CNT=${ESL_LANE_CNT} -DORCH_CASE=${ORCH_CASE} -DQWEN3_SPMD_TIER=${QWEN3_SPMD_TIER} ${ONBOARD_LOG_FLAGS} ${SWIMLANE_FLAGS} -Wno-error -w" \
    -DCMAKE_CXX_FLAGS="-DESL_PROXY_ONBOARD -DESL_LANE_CNT=${ESL_LANE_CNT} -DORCH_CASE=${ORCH_CASE} -DQWEN3_SPMD_TIER=${QWEN3_SPMD_TIER} ${ONBOARD_LOG_FLAGS} ${SWIMLANE_FLAGS} -Wno-error -w" \
...
if [[ "$ALL_CASES_FLAG" -ne 0 ]]; then
    ...
    OUT_ROOT="${ROOT}/report/swimlane/proxy/${DISPATCH_LABEL}"
