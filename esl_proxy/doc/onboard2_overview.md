...
**执行流程**（`main.c`）：
1. `esl_platform_init(&g_sim_runtime)` —— sim 平台初始化（runtime 布局、mem_pool、ring_buf、handshake）
2. `pthread_create` 启动 cutter / dispatch 线程
3. `aicpu_orchestration_entry(0)` —— 编排 case 构造 DAG
4. `atomic_store(&g_orch_is_done, true)` —— dispatch 进入 phase2
5. `pthread_join` 等待 cutter/dispatch
6. `esl_platform_shutdown(&g_sim_runtime)` —— shutdown AICore 寄存器
7. 校验 `g_completed_cnt == g_task_id`，打印 PASS/FAIL

**平台后端**：链接 `src/platform/sim/*.c`（含 `platform_init.c`）。`platform_*` 钩子为 no-op 或 host 原生实现；sim 无真 AICore 时由 `platform_handshake_aicore_bootstrap` 预填 handshake 字段（simpler sim 则 dlopen AICore SO 起线程，无需 prefill）。
...
- worker 映射：`platform_pick_phys_worker`
- 寄存器 MMIO / sim 桩：`read_reg`/`write_reg`（`platform_regs.h` + sim/onboard 各自实现）
- 缓存同步：`cache_invalidate_range`/`cache_flush_range`（sim: no-op；onboard: `dc civac`/`dc cvac`）
- tracing 钩子：`platform_cutter_loop_enter/iter`、`platform_dispatch_loop_exit` 等

两套后端实现：
- **sim**：`src/platform/sim/*.c`，全部 no-op 或 host 原生
- **onboard**：`src/platform/onboard/*.c`，真实 MMIO + DCCI 缓存操作

### 2.2 Handshake 与 platform init

- **algorithm**（`src/algorithm/handshake.c`）：`esl_handshake_start` → `platform_handshake_aicore_bootstrap` + `esl_handshake_all_cores`；无 `#ifdef ESL_PROXY_ONBOARD`
- **sim bootstrap**（`src/platform/sim/platform_init.c`）：无真 AICore，预填 `physical_core_id` / `aicore_regs_ready` / 假 reg table
- **onboard bootstrap**（`src/platform/onboard/platform_init.c`）：空实现，真 AICore 在 `aicore_executor.cpp` 写 ack 字段
- **统一入口**：`esl_platform_init` / `esl_platform_shutdown`（`platform.h`）；sim 与 onboard 各自 `platform_init.c`

dispatch 写 payload + `REG_ID_DATA_MAIN_BASE` 在 `dispatch.c` / `dispatch_payload.c`；sim `platform_regs.c` 在 `write_reg` 时记录 FIN。

### 2.3 onboard AICPU/AICore bring-up

- **AICPU 侧**（`src/platform/onboard/aicpu_runtime.c`、`platform_init.c`）：72 worker 模型，3 个 AICPU 线程角色（cutter/dispatch/orch），`esl_platform_init` 内含 handshake
...
### 2.4 调度快照同步（`ring_buf.h` + `dispatch.h`）

跨 AICPU 核共享调度状态（对齐 simpler AICPU↔AICPU 模型：atomic release/acquire + `wmb`）：

- [`ring_buf.h`](include/algorithm/ring_buf.h) `advance_task_id()` —— orch 侧：`wmb` + `atomic_fetch_add(g_task_id, release)`
- [`dispatch.h`](include/algorithm/dispatch.h) `sched_acquire_counters()` —— cutter/dispatch 每轮读前 acquire fence
- [`cutter.h`](include/algorithm/cutter.h) `extern g_commit_task_id` —— cutter 单写 commit 进度

consumer 侧（cutter `add_successors`、dispatch `dispatch_submit`）仍按需 per-slot `cache_invalidate_range` 读 orch 写的 slot 元数据。

sim 下 `wmb`/`cache_*` 为 compiler barrier 或 no-op。
...
### 3.1 `conf.h` —— 容量溢出修复（已应用）

| 修改 | 原值 | 新值 | 原因 |
|------|------|------|------|
| `NODE_BUFF_SIZE` | 8192 | **65536** | qwen3 attention DAG 前驱边数过多，8192 溢出前驱环 |
| `CON_NODE_CNT` | 32 | **256** | 共享 scope 生产者扇出过大，32 溢出后继图 |
...
写完后批次末 `wmb()` 发布前驱列表（v6 去掉 per-edge cvac，对齐 simpler atomic+wmb）。
...
`lock_q`/`unlock_q` 增加 `platform_queue_lock_prepare`/`platform_queue_unlock_publish`：onboard unlock 后 `wmb()` 发布队列变更；lock 侧依赖 `atomic_flag` acquire（v6 去掉 lock 前 civac）。sim 下 no-op。
