...
2. `platform_bringup()` —— sim 平台初始化（`EslRuntime` + handshake + `dispatch_bind`）
3. `pthread_create` 启动 cutter / dispatch 线程
4. `aicpu_orchestration_entry(0)` —— 编排 case 构造 DAG
5. `atomic_store(&g_orch_is_done, true)` —— dispatch 进入 phase2
6. `pthread_join` 等待 cutter/dispatch
7. `platform_teardown()` —— shutdown AICore 寄存器
...
### 2.2 AICore bridge（`include/algorithm/aicore_bridge.h` + `src/algorithm/aicore_bridge.c`）

sim/onboard 共用单一实现，API 直接传 `EslRuntime*`（无 `AicoreBridge` 结构体）：

- `aicore_bridge_shutdown(runtime)`
- `aicore_bridge_dispatch_task(runtime, tid, task_id, core, slot, exe_type, block_idx)` —— 写 payload + `REG_ID_DATA_MAIN_BASE`
- `aicore_bridge_poll_completions(runtime, tid)` —— 读完成状态，置位 `msg_bitmap`

sim：`platform_regs.c` 在 `write_reg` 时记录 FIN，dispatch 后可同步完成；onboard：`npu_hal.c` 读 `REG_ID_COND` 硬件完成寄存器。
