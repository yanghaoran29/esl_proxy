...
Tensor alloc_tensors(uint32_t shape[],int dim, int bytes) {
    size_t size = shape[0] * shape[1] * dim * bytes;
    return mem_pool_alloc(&g_mem_pool, size);
}

static inline void wait() {
    for (int i = 0; i < 10; i++)
    {
        asm volatile("dmd ish":::"memory");
    } 
}
/*
...
static inline void mem_pool_process_when2free(mem_pool_t *pool) {
    ...
    while (when2free_fifo_pop(&pool->fifo, &addr, &taskid) == 0) {
        ...
        /* Entry not eligible - this shouldn't happen in well-behaved system
         * since entries are processed in order and min_uncompleted only increases.
         * In practice, manager thread will re-check after processing all entries. */
    }
}
