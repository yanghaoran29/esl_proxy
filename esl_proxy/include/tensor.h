 ...
 * memcpy'd into TmEntry overlap fields (bytes [24,64) from start_offset
 * onward).
 */
...
/* External tensor: base + row-major layout (mirrors V200 from_tensor_arg). */
...
/* 2D subview: offsets/sizes on dim 0 and dim 1 (replaces tensor_view /
 * tensor_view_2d). */
static inline Tensor view(Tensor t, uint32_t off0, uint32_t off1, uint32_t n0, uint32_t n1) 
{
    ...
    v.extent_elem_cache = (v.shapes[0] - 1u) * v.strides[0] + (v.shapes[1] - 1u) * v.strides[1];
