...
enum class L2SwimlaneLevel : uint32_t {
    DISABLED = 0,       // No collection
    AICORE_TIMING = 1,  // AICore per-task start/end timestamps only
};
 ...
 * Instantiated as L2SwimlaneAicoreTaskBuffer below.
 */
...
enum class L2SwimlaneBufferKind : uint32_t {
    AicoreTask = 0,  // Per-core L2SwimlaneAicoreTaskBuffer, AICore writes, AICPU enqueues at rotation
};
...
inline size_t calc_l2_swimlane_data_size(int num_cores) {
    return sizeof(L2SwimlaneDataHeader) + static_cast<size_t>(num_cores) * sizeof(L2SwimlaneAicoreTaskPool);
}
