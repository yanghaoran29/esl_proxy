...
extern "C" void l2_swimlane_aicpu_init(int worker_count) {
    ...
    if (g_l2_swimlane_level != L2SwimlaneLevel::DISABLED &&
        g_l2_swimlane_level != L2SwimlaneLevel::AICORE_TIMING) {
        g_l2_swimlane_level = L2SwimlaneLevel::AICORE_TIMING;
    }
    ...
    wmb();
    if (head_table != nullptr && worker_count > 0) {
        cache_flush_range(head_table, static_cast<size_t>(worker_count) * sizeof(uint64_t));
    }
...
extern "C" void l2_swimlane_aicpu_on_aicore_dispatch(int core_id, int thread_idx) {
...
extern "C" void l2_swimlane_aicpu_flush(int thread_idx, const int *cur_thread_cores, int core_num) {
    if (!g_enable_l2_swimlane) {
        return;
    }
