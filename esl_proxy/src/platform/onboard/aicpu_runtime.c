...
int32_t esl_aicpu_execute(EslRuntime *runtime) {
    ...
    if (idx >= ESL_PROXY_AICPU_THREAD_NUM) {
        /* Excess launched thread (no-affinity fallback path): wait for the real
         * workers to finish; do NOT bump g_finished_count. */
    ...
    /* QuteMiao layout: 1 cutter + 1 dispatch + 1 orch. Role by idx. */
    switch (idx) {
