...
{
    return get_reg_ptr(reg_base_addr, REG_ID_COND);
}

void platform_ring_spec_doorbell(uint64_t reg_base_addr, uint32_t token)
{
    volatile uint64_t *dmb;

    dmb = (volatile uint64_t *)(uintptr_t)(reg_base_addr + reg_offset(REG_ID_DATA_MAIN_BASE));
    *dmb = ((uint64_t)token << 32) | (uint64_t)token;
