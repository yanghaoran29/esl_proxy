...
/* Precomputed COND-register pointers, resolved once here so the dispatch poll
 * reads the register without re-running reg_offset()/base-addition each spin
 * (mirrors simpler's core.cond_ptr). Parallel to g_core_reg_addrs. */
static volatile uint32_t *g_core_cond_ptr[RUNTIME_MAX_WORKER];
...
int esl_handshake_all_cores(EslRuntime *runtime)
{
    ...
    for (int i = 0; i < runtime->worker_count; i++) {
        ...
        g_core_cond_ptr[i] = platform_reg_cond_ptr(reg_addr);
...
volatile uint32_t *esl_handshake_cond_ptr(int core_idx)
{
    ...
    return g_core_cond_ptr[core_idx];
}
