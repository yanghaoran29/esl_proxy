...
void send_2_ready_queue(uint16_t ready_cnt[], uint16_t rq_buf[][LOCAL_BUFFER_SIZE]) {
    /* TODO: 仅发送 CUBE/VECTOR 队列；MIX 队列未接入，待支持后改 j < TASK_TYPE_CNT。 */
...
void resolve_dep(uint16_t cnt, uint16_t* cq_buf, uint16_t rq_buf[][LOCAL_BUFFER_SIZE], uint16_t* ready_cnt) {
    ...
    for (uint32_t j = 0; j < cnt; j++) {
        ...
        for (uint16_t k = 0; k < succ_cnt; k++) {
            ...
            pred_left = (uint16_t)(--g_predecessor_cnt[succ_id & RING_MASK]);
            wmb();
            WORKER_LOGF("cutter, task_id,%u, successor_id,%u, predecessor_cnt,%u", task_id, succ_id,
                        pred_left);
            if (pred_left < 1) {
                /* TODO: MIX 队列暂未实现，同 add_successors。 */
                int q = (int)g_basic_buf[succ_id].type;
...
void deal_completed_queue() {
    ...
    for (int i = 0; i < DISPATCH_THREAD_CNT; i++) {
        ...
        /* TODO: rq_buf/ready_cnt 仅覆盖 CUBE/VECTOR（大小 2）；MIX 队列未接入，
         *      一旦样例产生 MIX 任务会越界，待 MIX 支持后扩到 [3]。 */
    ...
    while (atomic_load_explicit(&g_commit_task_id, memory_order_acquire) <
        ...
        if (cur_commit != prev_commit) {
            ...
            if (stall > 10000) {
                ...
                esl_onboard_trace(ESL_AICPU_ROLE_CUTTER, ESL_TRACE_CUTTER_DRAIN,
                                  (uint64_t)cur_commit,
                                  (uint64_t)atomic_load_explicit(&g_task_id, memory_order_acquire),
                                  0xDEADBEEFULL);
                break;
