...
/* 依赖注入 AICore bridge。sim：传 NULL → dispatch_submit 走 fake return，
 * dispatch_poll 为 no-op。onboard：传真实 bridge，走硬件下发/完成路径。
 * 使 algorithm 层不依赖 onboard 头文件。 */
...
/* 把硬件 AICore 完成事件拉到 msg_bitmap。Onboard：aicore_bridge_poll_-
 * completions 读每个在役核的完成寄存器并置位对应 msg_bitmap bit，供
 * dispatch_drain_completions 解码。sim：g_bridge 为 NULL → no-op（fake
 * kernel 直接置 msg_bitmap）。 */
...
/* 核心动作一——向 AICore 下发一个任务。将 slot 占用更新（duration、idx、
 * task_id_map、free_bitmap 清位 + publish）与 bridge 下发调用捆绑，使 slot
 * 占用与下发原子完成。bridge 拒绝（返回非 0）时调用方回退入队。sim 路径
 * （g_bridge==NULL）直接置 msg_bitmap 模拟完成。 */
...
static inline int send_task(ctrl_t *ctrl, int type)
{
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        /* 下发任务给 AICore（核心动作一）：占用 slot + bridge 下发；
         * 被拒绝时回退入队此单任务并结束本轮。 */
        dispatch_submit(ctrl, type, exe_type, task_id, core, slot, mask);
