...
queue_t g_shared_ready[TASK_TYPE_CNT];
/* Per-producer fanout lock (simpler-style): serializes a producer P's
 * successor-list build (add_successors) against P's completion read
 * (resolve_dep). Distinct producers lock independently, so cutters resolve
 * different completions in parallel. Replaces the coarse global DAG lock. */
atomic_flag g_fanout_lock[RING_SIZE];
...
{
    /* Strided core ownership: lane tid owns cores { c : c % N == tid }. */
    uint64_t owned_mask[DISPATCH_THREAD_CNT];
    for (int tid = 0; tid < DISPATCH_THREAD_CNT; tid++) {
        owned_mask[tid] = 0;
    }
    for (int core = 0; core < AIC_CNT; core++) {
        owned_mask[CORE_LANE(core)] |= ((uint64_t)1 << core);
    }

    for (int i = 0; i < TASK_TYPE_CNT; i++) {
        memset(&g_shared_ready[i], 0, sizeof(queue_t));
        atomic_flag_clear_explicit(&g_shared_ready[i].lock, memory_order_release);
    }

    for (int tid = 0; tid < DISPATCH_THREAD_CNT; tid++) {
        g_ctrl_t[tid].tid = (uint16_t)tid;

        // Initialize free_bitmap for TASK_TYPE — only cores this lane owns
        for (int i = 0; i < TASK_TYPE_CNT; i++) {
            for (int j = 0; j < AIC_OSTD; j++) {
                g_ctrl_t[tid].free_bitmap[i][j] = owned_mask[tid];
            }
        }
        // set_mix(tid);
        ...
        // Initialize queues (ready tasks live in the shared g_shared_ready pool,
        // initialized below; ctrl_t only owns the per-lane completed_queue).
