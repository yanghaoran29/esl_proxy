// Orchestration: qwen3_decode static tensormap (tier-aware intra deps + tensormap inter-group).
//
// Same task graph as qwen3_decode_static.h; inter-group edges via tensormap lookup/insert.
                               ...
                               Tensor normed_tile, int64_t b0, int64_t cur_valid)
{
    ...
    Tensor v_view  = tensor_view(v_proj,      0u, (uint32_t)b0, QW3_TILE_ROWS);
    Tensor qn_view = tensor_view(q_proj_norm, 0u, (uint32_t)b0, QW3_TILE_ROWS);
    Tensor kn_view = tensor_view(k_proj_norm, 0u, (uint32_t)b0, QW3_TILE_ROWS);
    tm_tensor_insert(ly.v_last, &v_view);
    tm_tensor_insert(ly.qk_norm, &qn_view);
    tm_tensor_insert(ly.qk_norm, &kn_view);
...
static void apply_qw3_g2_group(int64_t b, int64_t b_tile0,
                               ...
                               int64_t slot_offset)
{
    ...
    const qw3_g2_layout_t ly = qw3_g2_build_group(
        base, b, b_tile0, (size_t)(b / QW3_TILE_ROWS), all_q_padded, ext_k_cache,
        ext_v_cache, k_proj_norm, ext_rope_cos, ext_rope_sin, v_proj, q_proj_norm,
        ext_block_table, all_raw_scores, all_exp_padded, all_cur_mi, all_cur_li,
        all_oi_tmp, attn_out, ctx_blocks, ctx_len, block_table_base, slot_block,
        slot_offset);

    Tensor kn_view = tensor_view(k_proj_norm, 0u, (uint32_t)b_tile0, QW3_TILE_ROWS);
    Tensor v_view  = tensor_view(v_proj,      0u, (uint32_t)b_tile0, QW3_TILE_ROWS);
    Tensor qn_view = tensor_view(q_proj_norm, 0u, (uint32_t)b_tile0, QW3_TILE_ROWS);
    tm_tensor_lookup(ly.rope, &kn_view, false);
    tm_tensor_lookup(ly.rope, &v_view, false);
    tm_tensor_lookup(ly.rope, &qn_view, false);

    Tensor ao_view = tensor_view(attn_out, 0u, (uint32_t)b, 1u);
    tm_tensor_insert(ly.online_last, &ao_view);
...
{
    (void)ext_hidden_states;
    (void)ext_wo;
    (void)ext_post_rms_weight;
    (void)ext_w_gate;
    (void)ext_w_up;
    (void)ext_w_down;
    (void)resid1_tile;
    (void)gm_pipe_buffer_0;
    (void)post_norm_tile;
    (void)gate_tile;
    (void)up_tile;
    (void)mlp_tile;
    ...
    Tensor ao_view = tensor_view(attn_out, 0u, (uint32_t)b0, (uint32_t)cur_valid);
    tm_tensor_lookup(ly.out_first, &ao_view, false);
    tm_tensor_insert(ly.down_res_last, &ext_out);
...
void aicpu_orchestration_entry(const uint64_t orch_args) {
    ...
    for (int64_t b = 0; b < user_batch; b += 1) {
        ...
        apply_qw3_g2_group(b, b_tile0, all_q_padded, ext_k_cache, ext_v_cache, k_proj_norm,
                         ext_rope_cos, ext_rope_sin, v_proj, q_proj_norm, ext_block_table,
                         all_raw_scores, all_exp_padded, all_cur_mi, all_cur_li, all_oi_tmp,
                         attn_out, ctx_blocks, ctx_len, block_table_base, slot_block,
                         slot_offset);
    }
    orch_build_flush();
